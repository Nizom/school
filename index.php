<?php
get_header();
?>
<div class="content-wrapper front">
	<div class="content">
		<div class="slider fotorama" data-width="100%"
     data-height="418px">
			<div>
				<img src="<?=get_template_directory_uri()?>/images/slider.png" alt="" class="slider-image image full-size">
				<p class="slider-text">Выпускные работы факультета<br>ландшафтного дизайна</p>
				<p class="slider-date">22 ноября 2015</p>
			</div>
			<div>
				<img src="<?=get_template_directory_uri()?>/images/slider.png" alt="" class="slider-image image full-size">
				<p class="slider-text">Выпускные работы факультета<br>ландшафтного дизайна</p>
				<p class="slider-date">20 ноября 2015</p>
			</div>
		</div>
		<div class="our-speciality">
			<p class="title-block">Наши специальности</p>
			<div class="quick-tabs">
				<ul class="quick-tabs-links">
					<li class="tab first active" data-tab="1"> <a href="#">Факультет менеджмента</a></li>
					<li class="tab second" data-tab="2"><a href="#">Факультет дизайна</a></li>
					<li class="tab third" data-tab="3"> <a href="#">Заочное обучение</a></li>
				</ul>
				<div id="tab_1" class="quick-tabs-container">
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
				</div>
				<div id="tab_2" class="quick-tabs-container">
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
				</div>
				<div id="tab_3" class="quick-tabs-container">
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
					<div class="quick-tabs-item">
						<div class="image-wrap">
							<img class="image show" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-show.png" alt="">
							<img class="image hover" src="<?=get_template_directory_uri()?>/images/quick-tabs-item-image-hover.png" alt="">
						</div>
						<p>Логистика<br>+ 1С торговля и склад</p>
					</div>
				</div>
			</div>
		</div>
	</div>
	<?php 
	get_sidebar();
	 ?>
	 <div class="form zayavka">
	 	<form action="" id="front-form">
	 	<div class="form-wrap">
		 	<p class="form-title">Заявка на обучение</p>
		 	<input type="text" class="form-input-text" placeholder="Ваше имя">
		 	<textarea name="message" id="" cols="30" rows="10" class="form-textarea" placeholder="Сообщение"></textarea>
		 	<input type="text" class="form-input-text" placeholder="Почта или телефон для ответа">
		 	<select name="speciality" id="front-speciality" class="form-input-select">
		 	<option disabled selected>Выберите специальность</option>
		 	<option value="test2">Тест</option>
		 	<option value="test3">Тест</option></select>
		</div>
		</form>
	 </div>
</div>
<?php
get_footer();
?>